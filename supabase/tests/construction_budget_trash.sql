...
BEGIN
 ...
 denied:=false;
 BEGIN PERFORM public.construction_budget_trash(p,(src->>'id')::uuid,'source',false); EXCEPTION WHEN raise_exception THEN denied:=true; END;
 IF NOT denied THEN RAISE EXCEPTION 'Source with live revision removed'; END IF;
 PERFORM public.construction_budget_trash(p,(rev->>'id')::uuid,'revision',false,1);
 IF public.construction_budget_load(p,(rev->>'id')::uuid)->>'deleted_at' IS NULL THEN RAISE EXCEPTION 'Revision not in trash'; END IF;
 denied:=false;
 BEGIN PERFORM public.construction_budget_save(p,(src->>'id')::uuid,(rev->>'id')::uuid,2,'Edit trash',doc,'[]',false); EXCEPTION WHEN serialization_failure THEN denied:=true; END;
 IF NOT denied THEN RAISE EXCEPTION 'Trashed revision editable'; END IF;
 ...
 IF NOT denied THEN RAISE EXCEPTION 'Restored revision without its source'; END IF;
 PERFORM public.construction_budget_trash(p,(src->>'id')::uuid,'source',true);
 PERFORM public.construction_budget_trash(p,(rev->>'id')::uuid,'revision',true,2);
 rev:=public.construction_budget_load(p,(rev->>'id')::uuid);
 IF rev->>'deleted_at' IS NOT NULL OR rev->>'version'<>'3' THEN RAISE EXCEPTION 'Restore failed'; END IF;
 rev:=public.construction_budget_save(p,(src->>'id')::uuid,(rev->>'id')::uuid,3,'Confirmed',doc,'[]',true);
 denied:=false;
 BEGIN PERFORM public.construction_budget_save(p,(src->>'id')::uuid,(rev->>'id')::uuid,4,'Overwrite confirmed',doc,'[]',false); EXCEPTION WHEN serialization_failure THEN denied:=true; END;
 IF NOT denied THEN RAISE EXCEPTION 'Confirmed revision changed'; END IF;
 PERFORM public.construction_budget_trash(p,(rev->>'id')::uuid,'revision',false,4);
 PERFORM public.construction_budget_trash(p,(rev->>'id')::uuid,'revision',true,5);
 IF public.construction_budget_load(p,(rev->>'id')::uuid)->>'status'<>'confirmed' THEN RAISE EXCEPTION 'Confirmation lost after restore'; END IF;
 ...
 denied:=false;
 BEGIN PERFORM public.construction_budget_trash(p,(rev->>'id')::uuid,'revision',false,6); EXCEPTION WHEN insufficient_privilege THEN denied:=true; END;
 IF NOT denied THEN RAISE EXCEPTION 'Cross-tenant trash allowed'; END IF;
END $$;
RESET ROLE;

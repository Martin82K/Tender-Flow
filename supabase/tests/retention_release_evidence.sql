...
BEGIN;
DO $$
DECLARE c public.contracts; test_id uuid := gen_random_uuid();
BEGIN
 SELECT c0.* INTO c FROM public.contracts c0 JOIN public.projects p ON p.id=c0.project_id
 WHERE p.owner_id IS NOT NULL LIMIT 1;
 IF c.id IS NULL THEN RAISE EXCEPTION 'No project fixture available'; END IF;
 PERFORM set_config('test.retention_contract_id',test_id::text,true);
 PERFORM set_config('test.retention_owner',(SELECT owner_id::text FROM public.projects WHERE id=c.project_id),true);
 ...
 VALUES(test_id,c.project_id,'Test','Rollback retention regression','active','CZK',1000,'manual',c.owner_id,c.organization_id,5,DATE '2026-08-01',0);

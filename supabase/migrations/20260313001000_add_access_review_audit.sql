-- Access review reporting and audit trail for role/permission changes.
...
AS $$
BEGIN
  IF TG_OP = 'UPDATE' AND COALESCE(OLD.role_id, '') IS DISTINCT FROM COALESCE(NEW.role_id, '') THEN
    PERFORM public.log_role_permission_audit_event(
      ...
      OLD.role_id,
      NEW.role_id,
      format('Aplikační role uživatele změněna z %s na %s', COALESCE(OLD.role_id, 'bez role'), COALESCE(NEW.role_id, 'bez role'))
    );
  END IF;
...
DROP TRIGGER IF EXISTS trg_audit_user_profile_role_change ON public.user_profiles;
CREATE TRIGGER trg_audit_user_profile_role_change
AFTER UPDATE OF role_id ON public.user_profiles
FOR EACH ROW
EXECUTE FUNCTION public.audit_user_profile_role_change();

CREATE OR REPLACE FUNCTION public.audit_org_member_role_change()
RETURNS TRIGGER
LANGUAGE plpgsql
SECURITY DEFINER
SET search_path = public
AS $$
BEGIN
  IF TG_OP = 'UPDATE' AND COALESCE(OLD.role, '') IS DISTINCT FROM COALESCE(NEW.role, '') THEN
    PERFORM public.log_role_permission_audit_event(
      'org_role_changed',
      NEW.user_id,
      NEW.role,
      ...
      OLD.role,
      NEW.role,
      format('Organizační role změněna z %s na %s', COALESCE(OLD.role, 'n/a'), COALESCE(NEW.role, 'n/a'))
    );
  END IF;
...
DROP TRIGGER IF EXISTS trg_audit_org_member_role_change ON public.organization_members;
CREATE TRIGGER trg_audit_org_member_role_change
AFTER UPDATE OF role ON public.organization_members
FOR EACH ROW
EXECUTE FUNCTION public.audit_org_member_role_change();
...
BEGIN
  IF TG_OP = 'INSERT' THEN
    PERFORM public.log_role_permission_audit_event(
      ...
      NULL,
      NEW.enabled::TEXT,
      format('Oprávnění %s pro roli %s nastaveno na %s', NEW.permission_key, NEW.role_id, NEW.enabled::TEXT)
    );
  ELSIF TG_OP = 'UPDATE' AND COALESCE(OLD.enabled, false) IS DISTINCT FROM COALESCE(NEW.enabled, false) THEN
    PERFORM public.log_role_permission_audit_event(
      'role_permission_changed',
      NULL,
      NEW.role_id,
      ...
      OLD.enabled::TEXT,
      NEW.enabled::TEXT,
      format('Oprávnění %s pro roli %s změněno z %s na %s', NEW.permission_key, NEW.role_id, OLD.enabled::TEXT, NEW.enabled::TEXT)
    );
  END IF;
...
AFTER INSERT OR UPDATE OF enabled ON public.role_permissions
FOR EACH ROW

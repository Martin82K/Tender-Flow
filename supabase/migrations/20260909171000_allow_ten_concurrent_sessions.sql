-- Allow ten interactive sessions and ten sessions per OAuth client per user.
-- Existing sessions, grants and the trigger are preserved; no backfill is needed.
...
AS $$
DECLARE
  MAX_FIRST_PARTY_SESSIONS CONSTANT INTEGER := 10;
  MAX_OAUTH_SESSIONS_PER_CLIENT CONSTANT INTEGER := 10;
...
COMMENT ON FUNCTION public.handle_new_session() IS
  'Limits first-party sessions to ten per user and OAuth sessions to ten per user/client without cross-bucket eviction.';

...
export const OrgOverviewTab: React.FC<OrgOverviewTabProps> = ({
  ...
  const trialDays = isTrial ? getCalendarDaysRemaining(subscription.billingPeriodEnd || subscription.expiresAt) : null;
  ...
  return (
    <section className="space-y-6">
      {/* Plan Summary Card */}
      <div data-help-id="org-plan-summary" className="bg-gradient-to-r from-primary/5 to-primary/0 border border-primary/20 rounded-xl p-6 flex flex-col sm:flex-row items-start sm:items-center gap-5">
        <div data-help-id="org-plan-icon" className="w-14 h-14 rounded-2xl bg-gradient-to-br from-primary to-primary/80 flex items-center justify-center text-white text-2xl flex-shrink-0">
        ...
        <div className="flex-1">
          <div className="flex items-center gap-3">
            ...
            <span className={`px-2.5 py-0.5 text-[11px] font-bold rounded border ${getTierBadgeClass(effectiveTier as any)}`}>
              {subscription.status === 'active' ? 'Aktivní' : isTrial ? 'Zkušební období' : subscription.status}
            </span>
          </div>
      ...
      {/* Seat Usage Bar */}
      {seatUsage && (
        <div data-help-id="org-seat-usage" className="bg-white dark:bg-slate-800 border border-slate-200 dark:border-slate-700/50 rounded-xl p-5">
          ...
          <div data-help-id="org-seat-progress" className="h-2 bg-slate-100 dark:bg-slate-700 rounded-full overflow-hidden">
            <div

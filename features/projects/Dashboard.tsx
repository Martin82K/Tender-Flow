...
export const Dashboard: React.FC<DashboardProps> = ({
  ...
  const [selectedProjectId, setSelectedProjectId] =
    useState<string>(getInitialProjectId);
  ...
  const handleExport = () => {
      ...
      .map((cat) => {
        ...
        const sodVr =
          bidInfo.winnersTotal > 0
            ? (cat.sodBudget || 0) - bidInfo.winnersTotal
            : null;
        const pnVr =
          bidInfo.winnersTotal > 0
            ? (cat.planBudget || 0) - bidInfo.winnersTotal
            : null;
    ...
    // Totals
    const totalSodAll = categories.reduce(
      (sum, c) => sum + (c.sodBudget || 0),
      0,
    );
    const totalPlanned = categories.reduce(
      (sum, c) => sum + (c.planBudget || 0),
      0,
    );
    const totalCenaVr = categories.reduce(
      (sum, cat) => sum + getCategoryBidInfo(cat.id).winnersTotal,
      0,
    );
    ...
    XLSX.writeFile(
      wb,
      `prehled-${selectedProject.title.replace(/\s+/g, "-")}.xlsx`,
    );
  };
  ...
  if (activeProjects.length === 0) {
    return (
      <div className="flex flex-col h-full overflow-y-auto bg-background-light dark:bg-background-dark">
        <Header title="Command Center" helpSlot={<HelpButton />} notificationSlot={<NotificationBell />} />
        <div className="flex-1 flex items-center justify-center">
          <div className="text-center">
            ...
            <p className="text-slate-500 mt-2">
              Přidejte projekt ve Správě staveb
            </p>
          </div>
        </div>
      </div>
  ...
  return (
    <div className="flex flex-col h-full bg-slate-50 dark:bg-slate-950">
      ...
      <div className="flex-1 overflow-auto bg-slate-50 dark:bg-slate-950">
        <div className="max-w-[1600px] mx-auto animate-fadeIn flex flex-col gap-6">
          <div className="px-6 md:px-8 pt-6">
            <CommandCenter />
          </div>
          <div data-help-id="dashboard-overview">
            {selectedProject ? (
              <ProjectOverviewNew

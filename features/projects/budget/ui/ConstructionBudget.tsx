...
export function ConstructionBudget({canUseTenders=false,projectId,organizationId,userId,categories:incomingCategories,readOnly=false,searchQuery='',onSearchChange}:Props) {
  ...
  const sources=useQuery({queryKey:[...key,'sources'],queryFn:()=>budgetApi.sources(projectId),enabled:!!index.data&&(tab==='versions'||repairOpen)});
  ...
  useEffect(()=>{
    ...
    if(previousActiveId.current&&previousActiveId.current!==activeId){
      ...
      setCategoryId('');setScopeOpen(false);setScopeSearch('');setPlanOpen(false);setRepairOpen(false);
    }
  ...
  const repairSource=sources.data?.find(s=>s.id===current?.source_id);
  const sourcesError=sources.error?<div role="alert" className="tf-budget-error">{sources.error.message}<button onClick={()=>void sources.refetch()}>Zkusit znovu</button></div>:null;
  const sourcesStatus=sources.isPending?<p role="status">Načítání příloh rozpočtu…</p>:!sources.data?sourcesError:null;
  ...
  return <section className={`tf-budget${view.grid?' tf-budget-show-grid':''}`} aria-label="Rozpočet stavby">
    ...
    <div className="tf-budget-toolbar tf-budget-commandbar" role="group" aria-label="Ovládání rozpočtu">
      ...
      {current&&(tab==='items'||tab==='recap')&&<>
        ...
        {editable&&<button onClick={()=>setRepairOpen(true)}>Opravit import</button>}
    ...
    {repairOpen&&current&&editable&&(sourcesStatus||!repairSource?<Modal isOpen title="Oprava importu rozpočtu" onClose={()=>setRepairOpen(false)}>{sourcesStatus??<p role="alert">Původní příloha není dostupná. Zkontrolujte Importy a verze.</p>}</Modal>:<BudgetImportDialog onCreateTender={permissions?.editTenders?createTender:undefined} categories={categories} canAllocate={permissions?.allocate??false} key={current.id} editRevision={current} projectId={projectId} source={repairSource} onClose={()=>setRepairOpen(false)} onComplete={r=>{setRepairOpen(false);if(r){cache.setQueryData([...key,'revision',r.id],r);setUndo({revisionId:current.id,document:current.document,allocations:current.allocations,version:r.version});}void cache.invalidateQueries({queryKey:key});}}/>)}

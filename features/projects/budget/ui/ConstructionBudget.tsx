...
export function ConstructionBudget({projectId,organizationId,userId,categories,readOnly=false,searchQuery='',onSearchChange}:Props) {
  ...
  const [jumpId,setJumpId]=useState('');const [columnsOpen,setColumnsOpen]=useState(false);const [catalogOpen,setCatalogOpen]=useState(false);
  ...
  const activeId=revisionId||index.data?.revisions.find(r=>!r.deleted_at)?.id||'';
  ...
  const save=async(document:BudgetDocument,allocations=current?.allocations??[],confirm=false)=>{
    ...
    try{const saved=await budgetApi.save({projectId,sourceId:current.source_id,revision:current,title:current.title,document,allocations,confirm});setUndo({document:current.document,allocations:current.allocations,version:saved.version});cache.setQueryData([...key,'revision',saved.id],saved);await cache.invalidateQueries({queryKey:[...key,'index']});setNotice('Uloženo na serveru.');return saved;}finally{saveLock.current=false;setSaving(false);}
  };
  ...
  const jump=(n:BudgetNode)=>{if(view.scope&&n.sheetId!==view.scope){updateView({scope:''});setNotice('Rozsah změněn na celý rozpočet kvůli vybranému oddílu. Sloupcové filtry zůstaly zachované.');}setTab('items');setJumpId(n.id);};
  ...
  return <section className="tf-budget" aria-label="Rozpočet stavby">
    ...
    <nav className="tf-budget-toolbar" aria-label="Sekce rozpočtu">{(['items','recap','versions'] as const).map(t=><button key={t} aria-current={tab===t?'page':undefined} onClick={()=>setTab(t)}>{t==='items'?'Položky':t==='recap'?'Rekapitulace':'Importy a verze'}</button>)}<button onClick={()=>setCatalogOpen(true)}>Firemní číselníky</button></nav>
    ...
    {tab==='versions'?<div className="overflow-auto p-4"><BudgetVersions revisions={index.data?.revisions??[]} sources={sources.data??[]} permissions={permissions!} purgeJobs={index.data?.purgeJobs} onPurge={async(jobId,selection)=>{try{await budgetApi.purge(projectId,jobId,selection);setRevisionId('');setSelected(new Set());setUndo(null);setNotice('Trvalé mazání dokončeno.');}finally{await cache.invalidateQueries({queryKey:key});}}} readOnly={readOnly} categoryNames={new Map(categories.map(c=>[c.id,c.title]))}
      onOpen={id=>{setRevisionId(id);setSelected(new Set());setUndo(null);setTab(index.data?.revisions.find(r=>r.id===id)?.deleted_at?'versions':'items');}}
      ...
      <h3>Historie otevřené verze</h3>{history.data?.map(h=><p key={h.id}>{new Date(h.created_at).toLocaleString('cs-CZ')} · {({trash:'Přesunuto do koše',restore:'Obnoveno z koše',create:'Vytvoření',save:'Úprava',confirm:'Potvrzení',apply_tender_plan:'Převzetí do plánu VŘ'} as Record<string,string>)[h.event]||h.event} · verze {h.previous_version??'-'} → {h.new_version}</p>)}
      ...
      <div className="tf-budget-toolbar">
        <button onClick={()=>updateView({panel:!view.panel})}>{view.panel?'Skrýt':'Zobrazit'} rekapitulaci</button><button onClick={()=>setScopeOpen(true)}>Rozsah: {sheets.find(s=>s.id===view.scope)?.title||'Celý rozpočet'} ▾</button>
        <label><input type="checkbox" role="switch" checked={view.showVV} onChange={e=>updateView({showVV:e.target.checked})}/>Výkaz výměr</label>
      ...
      <div className={`tf-budget-workspace ${tab==='recap'?'tf-budget-workspace-recap':''}`}>{(view.panel||tab==='recap')&&<BudgetRecap nodes={nodes} onJump={jump} prices={!!permissions?.prices}/>}{tab==='items'&&<BudgetTable figures={current.document.figures} nodes={nodes} scope={view.scope} filters={effectiveFilters} onFilters={next=>{const {$all,...columns}=next;setFilters(columns);}} selected={selected} onSelected={setSelected} showVV={view.showVV} wrap={view.wrap} density={view.density} columns={view.columns} onColumns={columns=>updateView({columns})} canPrices={!!permissions?.prices} editable={editable} jumpId={jumpId} onNotice={setNotice} onEdit={async edited=>{const document=applyBudgetItemEdit(current.document,edited);await save(document);}}/>}</div>
    ...
    {columnsOpen&&<Modal isOpen title="Nastavení sloupců" onClose={()=>{setColumnsOpen(false);viewOptionsButton.current?.focus();}}><div className="tf-budget-controls">{view.columns.map((c,i)=><div className="tf-budget-toolbar" key={c.key}><label><input type="checkbox" checked={!c.hidden} disabled={!c.hidden&&view.columns.filter(col=>!col.hidden).length===1} onChange={e=>updateView({columns:view.columns.map(col=>col.key===c.key?{...col,hidden:!e.target.checked}:col)})}/>{c.label}</label><label><input type="checkbox" checked={!!c.pinned} onChange={e=>updateView({columns:view.columns.map(col=>col.key===c.key?{...col,pinned:e.target.checked}:col)})}/>Připnout</label><button disabled={!i} onClick={()=>{const columns=[...view.columns];[columns[i-1],columns[i]]=[columns[i],columns[i-1]];updateView({columns});}}>Posunout vlevo</button></div>)}<button onClick={()=>updateView({columns:DEFAULT_COLUMNS})}>Výchozí sloupce</button></div></Modal>}
    ...
    {allocationOpen&&current&&<Modal isOpen title="Přiřazení množství do VŘ" persistent={saving} onClose={()=>setAllocationOpen(false)}><div className="tf-budget-controls flex flex-col gap-3"><p>{selected.size} položek. Existující alokace zůstanou zachované. Plán VŘ, nabídky ani smlouvy se nemění.</p><ThemedNativeSelect aria-label="Cílové VŘ" value={categoryId} onChange={e=>setCategoryId(e.target.value)}><option value="">Vyberte VŘ</option>{categories.map(c=><option key={c.id} value={c.id}>{c.title}</option>)}</ThemedNativeSelect>{selected.size===1&&<label>Množství (prázdné = celé množství)<input value={allocationQuantity} onChange={e=>setAllocationQuantity(e.target.value)}/></label>}<div className="max-h-48 overflow-auto">{current.document.nodes.filter(n=>selected.has(n.id)).map(n=><p key={n.id}>{n.code}: {n.quantity} {n.unit} · {current.allocations.filter(a=>a.itemId===n.id).map(a=>`${categories.find(c=>c.id===a.categoryId)?.title}: ${a.quantity}`).join(', ')||'Bez alokace'}</p>)}</div><button disabled={!categoryId||saving} onClick={()=>void act(async()=>{const additions=current.document.nodes.filter(n=>selected.has(n.id)&&isPriced(n)).map(n=>({itemId:n.id,categoryId,quantity:(selected.size===1&&allocationQuantity?decimal(allocationQuantity):n.quantity)??'0'}));await save(current.document,[...current.allocations,...additions]);setAllocationOpen(false);})}>Potvrdit přiřazení</button>{error&&<p role="alert">{error}</p>}</div></Modal>}

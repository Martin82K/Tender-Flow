...
interface Props { canAllocate?:boolean; editRevision?:BudgetRevision; projectId:string; source?:BudgetSource; previous?:BudgetRevision; hasVersions?:boolean; onClose:()=>void; onComplete:(revision?:BudgetRevision)=>void }
export function BudgetImportDialog({canAllocate=false,editRevision,projectId,source:initialSource,previous,hasVersions=!!previous,onClose,onComplete}:Props) {
  ...
  const publish=async()=>{
    ...
    try{
      ...
      if(transfer&&previous?.allocations.length&&!canAllocate)throw new Error('Přenos alokací vyžaduje oprávnění přiřazovat položky do VŘ.');
  ...
  return <Modal isOpen title={editor?'Oprava importu rozpočtu':document?'Kontrola importu':initialSource?'Převod uložené přílohy':'Import rozpočtu'} description={editor?'Zkontrolujte sloupce, strukturu a dopad oprav.':document?'Vyberte soupisy, které chcete zařadit do rozpočtu.':initialSource?'Zpracování uloženého XLSX do položek rozpočtu.':'Vyberte XLSX a způsob importu.'} onClose={()=>{controller.current?.abort();onClose();}} persistent={busy} showCloseButton={!busy} size={editor?"full":"lg"} footer={document?<div className="tf-budget-controls tf-budget-import-footer"><span>{selectedSheets} / {itemSheets.length} soupisů</span><button className="tf-budget-import-primary" disabled={busy||!title.trim()||!selectedSheets} onClick={()=>void publish()}>{busy?'Ukládání…':editor||editRevision?'Uložit a zavřít':mode==='assignments'||withTenders?'Pokračovat k přiřazení VŘ':hasVersions?'Vytvořit novou verzi':'Vytvořit rozpočet'}<ArrowRight size={14} aria-hidden="true"/></button></div>:!initialSource?<div className="tf-budget-controls tf-budget-import-footer"><span>{file||source?'Soubor je připravený k nahrání.':'Nejprve vyberte soubor.'}</span><button className="tf-budget-import-primary" disabled={busy||(!file&&!source)} onClick={()=>void start()}>{busy?<LoaderCircle size={16} className="tf-budget-spin" aria-hidden="true"/>:null}{busy?'Zpracovávám…':mode==='attachment'?'Uložit přílohu':'Nahrát a pokračovat'}{!busy&&<ArrowRight size={16} aria-hidden="true"/>}</button></div>:undefined}><div className="tf-budget-controls tf-budget-import flex flex-col gap-3">
    ...
    {document&&!editor&&<div className="tf-budget-import-review">
      ...
      {!editRevision&&canAllocate&&mode!=='assignments'&&<label><input type="checkbox" checked={withTenders} onChange={e=>setWithTenders(e.target.checked)}/>Převzít VŘ ze sloupců souboru</label>}

...
export function BudgetImportDialog({onCreateTender,tagOptions,categories=[],canImportTenders=false,canAllocate=false,editRevision,projectId,source:initialSource,previous:incomingPrevious,hasVersions=!!incomingPrevious,onClose,onComplete}:Props) {
  ...
  const [file,setFile]=useState<File|null>(null);const [mode,setMode]=useState<'attachment'|'convert'|'assignments'>('convert');const [source,setSource]=useState(initialSource);
  ...
  const selectFiles=(files:File[])=>{
    ...
    setFile(next);setAllocations([]);setSource(undefined);setDocument(null);setMapping({});setMappingSheet('');setPhase('');setError('');
  };
  const start=async(targetSheet?:string)=>{
    ...
    try{
      ...
      if(targetSheet&&document){
        ...
        if(allocations.some(a=>affected.has(a.itemId))||document.nodes.some(n=>affected.has(n.id)&&n.tags.length))throw new Error('Tento list má přiřazené štítky nebo množství. Změnu sloupců proveďte jako novou verzi s ověřeným přenosem vazeb.');
      }
  ...
  const linkedSheetIds=new Set((document?.nodes??[]).filter(n=>n.tags.length||allocatedIds.has(n.id)).map(n=>n.sheetId));
  const publish=async()=>{
  ...
  return <Modal isOpen title={editor?'Oprava importu rozpočtu':document?'Kontrola importu':initialSource?'Převod uložené přílohy':'Import rozpočtu'} description={editor?'Zkontrolujte sloupce, strukturu a dopad oprav.':document?'Vyberte soupisy, které chcete zařadit do rozpočtu.':initialSource?'Zpracování uloženého XLSX do položek rozpočtu.':'Vyberte XLSX a způsob importu.'} onClose={()=>{controller.current?.abort();onClose();}} persistent={busy} showCloseButton={!busy} size={editor?"full":"lg"} footer={document?<div className="tf-budget-controls tf-budget-import-footer"><span>{selectedSheets} / {itemSheets.length} soupisů</span><button className="tf-budget-import-primary" disabled={busy||!title.trim()||!selectedSheets} onClick={()=>void publish()}>{busy?'Ukládání…':editor||editRevision?'Uložit a zavřít':mode==='assignments'||withTenders?'Pokračovat k přiřazení VŘ':hasVersions?'Vytvořit novou verzi':'Vytvořit rozpočet'}<ArrowRight size={14} aria-hidden="true"/></button></div>:!initialSource?<div className="tf-budget-controls tf-budget-import-footer"><span>{file||source?'Soubor je připravený k nahrání.':'Nejprve vyberte soubor.'}</span><button className="tf-budget-import-primary" disabled={busy||(!file&&!source)} onClick={()=>void start()}>{busy?<LoaderCircle size={16} className="tf-budget-spin" aria-hidden="true"/>:null}{busy?'Zpracovávám…':mode==='attachment'?'Uložit přílohu':'Nahrát a pokračovat'}{!busy&&<ArrowRight size={16} aria-hidden="true"/>}</button></div>:undefined}><div className="tf-budget-controls tf-budget-import flex flex-col gap-3">
    ...
    {!document&&!initialSource&&<fieldset className="tf-budget-import-modes" disabled={busy}><legend>Jak chcete soubor použít?</legend>
      <label className={`tf-budget-import-option ${mode==='convert'?'is-selected':''}`}><input type="radio" name="budget-import-mode" checked={mode==='convert'} onChange={()=>setMode('convert')}/><FileSpreadsheet size={20} className="tf-budget-mode-icon" aria-hidden="true"/><span><strong>{hasVersions?'Aktualizovat rozpočet – nová verze':'Rozpočet do položek'}</strong><small>Položky převedete do rozpočtu. Před uložením zkontrolujete listy a jejich rozpoznání.</small></span><span className="tf-budget-radio-mark">{mode==='convert'&&<Check size={12} aria-hidden="true"/>}</span></label>
      ...
      {previous&&canImportTenders&&canAllocate&&<label className={`tf-budget-import-option ${mode==='assignments'?'is-selected':''}`}><input type="radio" name="budget-import-mode" checked={mode==='assignments'} onChange={()=>setMode('assignments')}/><FileSpreadsheet size={20} className="tf-budget-mode-icon" aria-hidden="true"/><span><strong>Pouze převzít přiřazení do VŘ</strong><small>Ceny a množství zůstanou z cílové revize {previous.title}.</small></span><span className="tf-budget-radio-mark">{mode==='assignments'&&<Check size={12} aria-hidden="true"/>}</span></label>}
    </fieldset>}
    ...
    {document&&editor&&<BudgetImportEditor onCreateTender={onCreateTender} tagOptions={tagOptions} allocations={allocations} categories={categories} canAllocate={canAllocate&&!busy} onAllocate={async(itemId,categoryId)=>{
      if(!canAllocate||busy)throw new Error('Přiřazení není povoleno.');

...
export interface ImportIssue { sheet: string; row: number; severity: 'warning' | 'error'; message: string; kind?: 'ambiguous-figures' | 'hierarchy' | 'unclassified'; figures?: FigureConflict[] }
export interface BudgetImportRepair { nodeId: string; parentId: string; kind: Exclude<BudgetNodeKind, 'object' | 'sheet'>; scope: 'row' | 'subtree' }
export interface BudgetDocument { schemaVersion: 1; origin?: 'import' | 'copy'; importKey?: string; sheets: BudgetSheet[]; nodes: BudgetNode[]; issues: ImportIssue[]; figures: Record<string, string>; figureResolutions?: Record<string, FigureResolution>; importRepairs?: BudgetImportRepair[] }

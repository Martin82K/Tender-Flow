...
export function OfferComparisonPanel({ projectId, categoryId, categoryTitle, resolveFolder, onClose }: Props) {
    ...
    useEffect(() => { alive.current = true; comparisonApi.index(projectId).then(result => { if (alive.current) {
        ...
        setViews(result.views.filter(v => v.category_id === categoryId || v.category_id === null));
    ...
    const inspect = async (file: File) => {
        if (/\.pdf$/i.test(file.name)) {
            if (!document.sources.length) throw new Error('Pro základnu vyberte poptávkový XLSX nebo revizi rozpočtu. PDF lze přidat jako nabídku k ověření.');
    ...
    const results = document.sources.slice(1).map(source => {
        const assignments = document.assignments[source.id] || [];
        return { source, result: compareOffer(base.items, source.items, assignments), itemsById: new Map(source.items.map(item => [item.id, item])), linksByBase: new Map(assignments.map(link => [link.baseId, link])), usedOffers: new Set(assignments.map(link => link.offerId)) };
    });
    const save = () => void run(async () => {
        ...
        const result = await comparisonApi.save(projectId, saved ? saved.category_id : categoryId, title, document, request.current, saved);
        if (alive.current) {
            setSaved(result);
            setViews((await comparisonApi.index(projectId)).views.filter(v => v.category_id === categoryId || v.category_id === null));
        }
    });
    ...
    <div className="flex flex-wrap gap-3"><label>Uložené porovnání <ThemedNativeSelect aria-label="Uložené porovnání" className={inputClass} value={saved?.id || ''} disabled={busy} onChange={e => { const id = e.target.value; setDeleteConfirm(false); if (!id) {
        setSaved(undefined);
        ...
        void run(async () => { const result = await comparisonApi.load(projectId, id); if (alive.current) {
            ...
            setPending(null);
            setPage(0);
        } }); }}><option value="">Nové porovnání</option>{views.map(v => <option key={v.id} value={v.id}>{v.title}{v.category_id === null ? ' · projektový pohled' : ''}</option>)}</ThemedNativeSelect></label>
    ...
    {saved && <button type="button" className={inputClass} disabled={!canEdit || busy} onClick={() => setDeleteConfirm(true)}>Smazat pohled</button>}
    {saved && deleteConfirm && <div role="alert" className="rounded border border-amber-300 p-3"><p>Smazat uložené porovnání „{saved.title}“? Původní soubory, nabídky a rozpočet zůstanou zachované.</p><button type="button" className={inputClass} disabled={!canEdit || busy} onClick={() => void run(async () => {
        await comparisonApi.remove(projectId, saved.id, saved.version);
        if (!alive.current) return;
        setViews(previous => previous.filter(view => view.id !== saved.id));
        setSaved(undefined); setDeleteConfirm(false); setDocument({ schemaVersion: 1, sources: [], assignments: {} });
        setSuggestions({}); setRowSearch({}); setPending(null); setPendingPdf(null); setPage(0);
        setTitle(`Porovnání — ${categoryTitle}`); request.current = crypto.randomUUID();
    })}>Potvrdit smazání pohledu</button> <button type="button" className={inputClass} disabled={busy} onClick={() => setDeleteConfirm(false)}>Zrušit smazání</button></div>}
    ...
    {pending && <fieldset className="space-y-3 rounded border p-3"><legend>Mapování: {pending.name}</legend>{pending.parsed.workbook.SheetNames.filter(name => !pending.parsed.mappings.some(mapping => mapping.sheet === name)).map(name => <div key={name} className="text-sm text-amber-800 dark:text-amber-300">List {name} není zahrnutý. <button type="button" className={inputClass} onClick={() => setPending({ ...pending, parsed: { ...pending.parsed, mappings: [...pending.parsed.mappings, { sheet: name, headerRow: 1, columns: {} }] } })}>Přidat list: {name}</button></div>)}{pending.parsed.mappings.map((m, i) => <div key={i} className="flex flex-wrap gap-3"><label>List <ThemedNativeSelect className={inputClass} value={m.sheet} onChange={e => setPending({ ...pending, parsed: { ...pending.parsed, mappings: pending.parsed.mappings.map((x, n) => n === i ? { ...x, sheet: e.target.value } : x) } })}>{pending.parsed.workbook.SheetNames.map(s => <option key={s}>{s}</option>)}</ThemedNativeSelect></label><label>Řádek hlavičky <input type="number" min={1} max={10000} className={`${inputClass} w-20`} value={m.headerRow} onChange={e => setPending({ ...pending, parsed: { ...pending.parsed, mappings: pending.parsed.mappings.map((x, n) => n === i ? { ...x, headerRow: Number(e.target.value) } : x) } })}/></label>{(Object.keys(labels) as OfferColumn[]).map(role => <label key={role}>{labels[role]} <input aria-label={`${m.sheet} ${labels[role]}`} type="number" min={1} max={128} placeholder="—" className={`${inputClass} w-20`} value={m.columns[role] === undefined ? '' : m.columns[role]! + 1} onChange={e => setPending({ ...pending, parsed: { ...pending.parsed, mappings: pending.parsed.mappings.map((x, n) => n === i ? { ...x, columns: { ...x.columns, [role]: e.target.value ? Number(e.target.value) - 1 : undefined } } : x) } })}/></label>)}<button type="button" className={inputClass} onClick={() => setPending({ ...pending, parsed: { ...pending.parsed, mappings: pending.parsed.mappings.filter((_, index) => index !== i) } })}>Odebrat list: {m.sheet}</button></div>)}<p className="text-xs">Čísla sloupců: A = 1, B = 2… Prázdná role se nepoužije.</p><button className={inputClass} onClick={acceptSource}>Potvrdit mapování</button> <button className={inputClass} onClick={() => setPending(null)}>Zrušit</button></fieldset>}
    {base && <><p className="text-sm">Základna: <strong>{base.name}</strong> · {base.items.length} položek · ceny v původní měně a režimu DPH dokumentů; před porovnáním ověřte jejich shodu.</p><div className="max-h-[60vh] overflow-auto rounded border"><table className="w-full border-collapse text-sm"><thead className="sticky top-0 z-20 bg-slate-100 dark:bg-slate-800"><tr><th className="md:sticky left-0 z-30 min-w-[100px] bg-slate-100 p-2 dark:bg-slate-800">Kód</th><th className="md:sticky left-[100px] z-30 min-w-[360px] bg-slate-100 p-2 dark:bg-slate-800">Položka / objekt</th><th className="md:sticky left-[460px] z-30 min-w-[136px] bg-slate-100 p-2 dark:bg-slate-800">Množství</th>{results.map(({ source, result }) => <th key={source.id} className="min-w-64 border-l p-2">{source.name}<div><button type="button" className={inputClass} disabled={!canEdit || busy || !aiConsent} onClick={() => void run(async () => { const proposed = await suggestOfferMatches(projectId, base.items, source.items, document.assignments[source.id] || []); if (alive.current)
        setSuggestions(previous => ({ ...previous, [source.id]: proposed })); })}>Navrhnout nejasné shody</button></div><div className="font-normal">{result.pricedCount}/{base.items.length} srovnatelných cen · {result.pricedCount ? result.total : '—'}{!result.complete && ' (dílčí)'}</div></th>)}</tr></thead><tbody>{base.items.slice(page * 100, (page + 1) * 100).map((item, offset) => { const index = page * 100 + offset; return <tr key={item.id} className="border-b odd:bg-white even:bg-slate-100 dark:odd:bg-slate-900 dark:even:bg-slate-800"><td className="md:sticky left-0 z-10 min-w-[100px] max-w-[100px] bg-inherit p-2">{item.code}</td><td className="md:sticky left-[100px] z-10 min-w-[360px] max-w-[360px] bg-inherit p-2"><span className="text-xs text-slate-500">{item.group}</span><div>{item.description}</div>{item.note && <p className="mt-1 text-xs text-amber-800 dark:text-amber-300">{item.note}</p>}</td><td className="md:sticky left-[460px] z-10 min-w-[136px] max-w-[136px] whitespace-nowrap bg-inherit p-2">{item.quantity} {item.unit}</td>{results.map(({ source, result, itemsById, linksByBase, usedOffers }) => { const row = result.rows[index], link = linksByBase.get(item.id), matchedItem = row.offerId ? itemsById.get(row.offerId) : undefined; const search = rowSearch[`${source.id}:${item.id}`]?.toLocaleLowerCase('cs'); const candidates = search ? source.items.filter(offer => `${offer.code} ${offer.description}`.toLocaleLowerCase('cs').includes(search)) : (link?.candidates || []).flatMap(id => { const offer = itemsById.get(id); return offer ? [offer] : []; }); const options = [...(matchedItem ? [matchedItem] : []), ...candidates.filter(offer => offer.id !== link?.offerId && !usedOffers.has(offer.id)).slice(0, 50)]; return <td key={source.id} className="border-l p-2">{suggestions[source.id]?.filter(s => s.baseId === item.id).map(s => <p key={s.baseId} className="text-xs text-amber-700 dark:text-amber-300">AI návrh: {s.offerId ? itemsById.get(s.offerId)?.description : 'Bez jisté shody'} — {s.reason}. Ověřte a vyberte položku níže. <button disabled={busy} onClick={() => void run(() => reviewSuggestion(s.runId, s.baseId, true))}>Návrh správný</button> / <button disabled={busy} onClick={() => void run(() => reviewSuggestion(s.runId, s.baseId, false))}>Návrh chybný</button></p>)}<div>{row.quotedTotal ?? (row.priceStatus === 'missing-price' ? 'Chybí cena' : 'Nespárováno')}{row.priceStatus === 'different-scope' && ' · jiné množství / MJ'}</div>{matchedItem?.note && <p className="my-1 text-xs text-amber-800 dark:text-amber-300">{matchedItem.note}</p>}<input aria-label={`Hledat položku: ${source.name} ${item.code}`} placeholder="Hledat kód nebo popis…" className={`${inputClass} max-w-72`} value={rowSearch[`${source.id}:${item.id}`] || ''} onChange={e => setRowSearch(previous => ({ ...previous, [`${source.id}:${item.id}`]: e.target.value }))}/><ThemedNativeSelect aria-label={`${source.name}: ${item.code} ${item.description}`} className={`${inputClass} max-w-72`} value={link?.offerId || ''} disabled={!canEdit || busy} onChange={e => { const offerId = e.target.value || null; setDocument(previous => ({ ...previous, assignments: { ...previous.assignments, [source.id]: [...(previous.assignments[source.id] || []).filter(a => a.baseId !== item.id), { baseId: item.id, offerId, status: offerId ? 'manual' : 'unmatched' }] } })); }}><option value="">{link?.status === 'review' ? 'Ověřit shodu…' : 'Bez přiřazení'}</option>{options.map(o => <option key={o.id} value={o.id}>{o.code} · {o.description.slice(0, 70)} · {o.quantity} {o.unit} · {o.source.sheet}:{o.source.row}</option>)}</ThemedNativeSelect></td>; })}</tr>; })}</tbody></table></div><div className="flex gap-3"><button className={inputClass} disabled={page === 0} onClick={() => setPage(p => p - 1)}>Předchozí</button><span>Řádky {page * 100 + 1}–{Math.min((page + 1) * 100, base.items.length)}</span><button className={inputClass} disabled={(page + 1) * 100 >= base.items.length} onClick={() => setPage(p => p + 1)}>Další</button></div>{results.map(({ source, result, itemsById }) => <details key={source.id}><summary>{source.name}: {result.extraIds.length} položek bez protějšku · {source.notes.length} původních poznámek</summary><ul>{result.extraIds.map(id => <li key={id}>{itemsById.get(id)?.description}{itemsById.get(id)?.note && ` — ${itemsById.get(id)?.note}`}</li>)}{source.notes.map((note, i) => <li key={`note-${i}`}>{note}</li>)}</ul></details>)}</>}
  </section>;
}

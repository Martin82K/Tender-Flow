...
export function DocumentDetail({ version, versions, vendorId, canWrite, onBack, onEdit, onRefresh, initialEditor = false }: {
  ...
  return <div className="space-y-5">
    ...
    <div className="flex flex-wrap justify-between gap-3"><div><p className="text-xs text-slate-500">{documentKindLabels[version.snapshot.kind]} · Verze {version.version}</p><h3 className="text-xl font-semibold mt-1">{fields.recordTitle || documentKindLabels[version.snapshot.kind]}</h3><p className="text-sm text-slate-500 mt-1">{fields.vendorName} · {fields.contractNumber || 'Bez čísla smlouvy'}</p></div><div className="flex flex-wrap gap-2 items-start"><Button size="sm" variant="outline" onClick={() => setEditor({version, readonly:true})}>Náhled a export</Button>{canWrite && <><Button size="sm" disabled={busy} variant="outline" onClick={onEdit}>Upravit záznam</Button><Button size="sm" variant="danger" disabled={busy} onClick={() => { setError(''); setDeleteOpen(true); }}>Smazat záznam</Button><Button size="sm" onClick={() => setEditor({version, readonly:false})}>Otevřít editor</Button></>}</div></div>
    ...
    {deleteOpen && canWrite && <Modal isOpen persistent={busy} onClose={() => { if (!busy) setDeleteOpen(false); }} title="Smazat předávací protokol" size="lg">
      <div className="space-y-4"><p>Smazat záznam „{fields.recordTitle || documentKindLabels[version.snapshot.kind]}“? Zmizí ze seznamu protokolů. Historie verzí, přílohy a potvrzení skutečného předání zůstanou uchované pro audit.</p>
        {error && <p role="alert" className="text-red-600">{error}</p>}
        <div className="flex justify-end gap-2"><Button variant="outline" disabled={busy} onClick={() => setDeleteOpen(false)}>Zrušit</Button><Button variant="danger" disabled={busy} onClick={() => void run(async () => { await contractDocumentsApi.remove(version.contract_id, version.document_id, version.version); await onRefresh(); onBack(); })}>{busy ? 'Mažu…' : 'Smazat záznam'}</Button></div>
      </div>
    </Modal>}

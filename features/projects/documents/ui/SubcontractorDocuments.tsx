...
export function SubcontractorDocuments({ projectId, contractsState, readOnly }: { projectId: string; contractsState: UseContractsWithDetailsResult; readOnly: boolean }) {
  ...
  return <div className="space-y-6">
    <div className="flex flex-wrap items-center justify-between gap-3"><div><h2 className="text-2xl font-bold">Subdodavatel</h2><p className="mt-1 text-sm text-slate-500">Dokumenty stavby · Předání staveniště a díla</p></div>{!readOnly && <Button disabled={!canWrite || loading || !contracts.length} onClick={() => setForm('new')}>Nový záznam</Button>}</div>
    {!readOnly && permission.isPending && <p role="status" className="text-sm text-slate-500">Ověřuji oprávnění k úpravě protokolů…</p>}
    {!readOnly && permission.data === false && <p className="text-sm text-slate-500">K vytváření a úpravám protokolů potřebujete oprávnění upravovat smlouvy této stavby.</p>}
    ...
    <div role="tabpanel" id="documents-panel" aria-labelledby={`documents-tab-${view}`}>
      {view==='other' ? <DevelopmentPlaceholder title="Ostatní dokumenty" /> : error ? <div role="alert" className="space-y-3"><p className="text-red-600">{error}</p><Button variant="outline" onClick={() => { void contractsState.refresh(); void versions.refetch(); void permission.refetch(); }}>Zkusit znovu</Button></div> : loading ? <p role="status">Načítám dokumenty…</p> : selected ? <DocumentDetail key={selected.document_id} version={selected} vendorId={contracts.find(contract => contract.id === selected.contract_id)?.vendorId} versions={(versions.data || []).filter(v => v.document_id===selected.document_id)} canWrite={canWrite} initialEditor={openEditorFor===selected.document_id} onBack={() => go('protocols')} onEdit={() => setForm(selected)} onRefresh={refresh} /> : documentId ? <p role="alert">Dokument není dostupný na této stavbě. <button className="text-primary" onClick={() => go('protocols')}>Zpět na seznam</button></p> : <div className="space-y-4">
        ...
        {view==='protocols' && !contracts.length ? <div className="space-y-3"><p className="text-sm text-slate-500">Pro vytvoření protokolu nejprve přidejte subdodavatelskou smlouvu.</p><Button variant="outline" onClick={() => navigate(buildAppUrl('project', {projectId, tab:'contracts'}))}>Přejít na smlouvy</Button></div> : view==='protocols' && !visible.length && <p className="text-sm text-slate-500">Žádné odpovídající protokoly.{canWrite ? ' Vytvořte nový záznam nebo upravte filtry.' : ' Upravte filtry nebo požádejte správce stavby o oprávnění.'}</p>}
      </div>}
    </div>

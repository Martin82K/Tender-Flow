...
export const DrawdownsSection: React.FC<{ contract: ContractWithDetails }> = ({ contract }) => {
  ...
  return (
    <section id="sec-cerpani" className="py-4 border-b border-dashed border-slate-200 dark:border-slate-800">
      ...
      <div className="flex flex-wrap gap-4 mb-3">
        <div>
          <div className="text-[10.5px] uppercase tracking-wider text-slate-600 dark:text-slate-500">Hodnota smlouvy</div>
        ...
        <div>
          <div className="text-[10.5px] uppercase tracking-wider text-slate-600 dark:text-slate-500">Vyčerpáno</div>
        ...
        <div className="w-px bg-slate-200 dark:bg-slate-800 self-stretch" />
        <div>
          <div className="text-[10.5px] uppercase tracking-wider text-slate-600 dark:text-slate-500">Zbývá</div>
          <div className="text-sm font-semibold text-slate-900 dark:text-slate-100 tabular-nums">
          ...
          <tbody>
            {contract.drawdowns.map((d) => (
              <tr key={d.id} className="border-b border-slate-200 dark:border-slate-800">
                <td className="px-2.5 py-2 text-slate-900 dark:text-slate-200">{d.period}</td>
                <td className="px-2.5 py-2 text-right tabular-nums text-slate-700 dark:text-slate-300">
                ...
                <td className="px-2.5 py-2 text-slate-600 dark:text-slate-400">{d.note || '—'}</td>
              </tr>
            ))}
          </tbody>
        </table>

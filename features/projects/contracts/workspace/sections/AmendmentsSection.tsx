...
export const AmendmentsSection: React.FC<Props> = ({ contract, onRefresh }) => {
  ...
  return (
    <section id="sec-dodatky" className="py-4 border-b border-dashed border-slate-200 dark:border-slate-800">
            ...
            <tbody>
              {contract.amendments.map((a) => (
                <tr key={a.id} className="border-b border-slate-200 dark:border-slate-800">
                  <td className="px-2.5 py-2 text-slate-900 dark:text-slate-200">Dodatek č. {a.amendmentNo}</td>
                  <td className="px-2.5 py-2 text-slate-600 dark:text-slate-400">{formatDate(a.signedAt)}</td>
                  <td

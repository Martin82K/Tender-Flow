...
export const ContractListPanel: React.FC<Props> = ({ contracts, selectedId, onSelect }) => {
  ...
  return (
    <aside className="flex flex-col rounded-xl border border-slate-800 bg-slate-900/60 overflow-hidden">
          ...
          visible.map((c) => {
            ...
            return (
              <button
                ...
                className={`w-full text-left p-3 border-b border-slate-800 transition ${
                  isActive
              ...
              >
                <div className="flex items-center justify-between gap-2">
                  <div className="min-w-0">
                    <div className="font-semibold text-slate-100 text-sm truncate">{c.title}</div>
                    {c.contractNumber && (
                      <div className="text-[10.5px] text-slate-500">{c.contractNumber}</div>
                    )}
                  </div>
                  <StatusPill status={c.status} />
                </div>
                <div className="text-xs text-slate-400 mt-1 truncate">{c.vendorName}</div>
                <div className="flex items-center justify-between mt-2 text-[11px]">
                  ...
                  <span className="text-slate-500">
                    ...
                    {c.overdueSum > 0 && (
                      <span className="inline-block rounded-full bg-red-500/15 text-red-400 px-2 py-0.5 text-[10px] font-semibold">
                        po splat.
                      </span>
                    )}
                  </span>
                </div>
                <div className="mt-1.5 h-[3px] rounded bg-slate-800 overflow-hidden">
                  <span
                    className="block h-full bg-gradient-to-r from-amber-500 to-green-500"
                    style={{ width: `${pct}%` }}
                  />
                </div>
                <div className="text-[10.5px] text-slate-500 mt-0.5">Nafakturováno {pct} %</div>
              </button>
            );
          })
        )}

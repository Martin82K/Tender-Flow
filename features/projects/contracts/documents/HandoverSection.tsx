...
export const HandoverSection: React.FC<{ contract: ContractWithDetails; onRefresh: () => Promise<void> | void }> = ({ contract, onRefresh }) => {
  ...
  return <section className="py-4 space-y-4">
    ...
    {kind && <form onSubmit={confirm} className="rounded-xl border border-primary/40 p-4 space-y-3">
    ...
    {!!events.data?.length && <details className="text-xs"><summary className="cursor-pointer text-slate-500">Historie potvrzení ({events.data.length})</summary><ol className="space-y-3 mt-3">{events.data.map(event => <li key={event.id} className="border-l-2 border-slate-300 dark:border-slate-700 pl-3"><strong>{event.kind === 'handover' ? 'Předání' : 'Začátek záruky'} · {formatDate(event.effective_date)}</strong><p className="whitespace-pre-wrap break-words mt-1">{event.source_note}</p><p className="text-slate-500 mt-1">Zapsáno {new Date(event.created_at).toLocaleString('cs-CZ')} · Autor {event.created_by || 'Odstraněný účet'}</p></li>)}</ol></details>}
  </section>;
};

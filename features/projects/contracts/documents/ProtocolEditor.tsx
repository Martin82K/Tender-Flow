...
import { documentFileName, freezeDocument, resultLabels, documentKindLabels, type DocumentKind, type DocumentLogo, type DocumentVersion, type HandoverFields } from './model';
...
export const ProtocolEditor: React.FC<Props> = ({ contractId, initialFields, logo, existing, kind = existing?.snapshot.kind || 'sub_work_handover', previewOnly, vendorContacts = [], onClose, onSaved }) => {
  ...
  const save = async (showPreview: boolean) => {
    ...
    try {
      if (dirty || !saved) {
        const result = await contractDocumentsApi.save(contractId, documentId, saved?.version || 0, freezeDocument(fields, new Date().toISOString(), (saved?.version || 0) + 1, logo, kind));
  ...
  return <Modal isOpen onClose={close} persistent={busy} size="2xl" title={`${documentKindLabels[kind]} · ${fields.contractNumber || fields.vendorName}`} footer={<div className="flex flex-wrap gap-2 w-full">
    ...
    {preview ? <div className="space-y-3"><p className="text-xs text-slate-500">Verze {saved?.version} · Finální PDF nebo upravený DOCX můžete připojit v záložce Dokumenty.</p>{pdfUrl ? <iframe className="w-full h-[65vh] rounded-lg border border-slate-300" title="Náhled předávacího protokolu" src={pdfUrl} /> : <p role="status">Připravuji náhled…</p>}</div> : <fieldset disabled={busy} className="max-w-4xl mx-auto space-y-5">
      ...
      {kind === 'sub_site_handover' && <>{field('siteConditions', true)}{field('siteSafety', true)}{field('siteFacilities', true)}</>}
      {field('defects', true)}
      ...
      {field('note', true)}
      <p className="text-xs text-slate-500">Uložení protokolu nepotvrzuje předání. Skutečné předání zapíšete v detailu dokumentu; začátek záruky samostatně ve smlouvě.</p>
    </fieldset>}
  </Modal>;
};

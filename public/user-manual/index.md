...
- [🤖 Viki – AI asistentka](#viki-ai-asistentka)
- [🔧 Nástroje](#nastroje)
...
> 💡 **Tip:** Kdykoli potřebujete pomoc, otevřete **Viki** (AI asistentku) – poradí vám s projektem, rozpočtem i přípravou emailu.
...
Viki je inteligentní asistentka integrovaná přímo v aplikaci Tender Flow. Pomáhá s analýzou projektů, přípravou emailů a rychlým přehledem stavu výběrových řízení.

> 💡 **Tip:** Viki je dostupná od tarifu **Enterprise**. Pokud ikonu Viki nevidíte, zkontrolujte úroveň předplatného.

### Jak Viki otevřít a používat

1. Klikněte na plovoucí ikonu v pravém dolním rohu obrazovky.
2. Otevře se panel Viki s konverzačním rozhraním.
3. Napište svůj dotaz nebo požadavek do textového pole.
4. Viki automaticky rozpozná, jakou dovednost použít, na základě klíčových slov ve vaší zprávě.
5. Pokud žádná dovednost neodpovídá, Viki odpoví pomocí obecného AI modelu.

Panel Viki obsahuje:
- **Konverzaci** – historie zpráv a odpovědí.
- **Nastavení** (ikona ozubeného kola) – kontextové scopy, režim, hlasové ovládání.

### Dovednosti (Skills)

Viki disponuje pěti specializovanými dovednostmi, které se automaticky aktivují podle klíčových slov:

#### Projektový briefing

Rychlé shrnutí aktivního projektu včetně financí a postupu.

**Aktivační slova:** briefing, shrnutí projektu, stav projektu, přehled projektu, co je nového na projektu

**Co vrátí:** lokaci, fázi, počet kategorií, finanční delta (interní plán vs. investor SOD), doporučené akce.

#### Detailní reporting projektu

Nestranný detailní report projektu s KPI, riziky, tabulkami a grafy.

**Aktivační slova:** detailní briefing, detailní report, podrobný report, KPI projektu, rizika projektu, zhodnocení projektu

**Co vrátí:** kompletní analýzu včetně rozpočtové bilance, stavu kategorií (Otevřeno / V jednání / Uzavřeno / SOD), počtu nabídek, ASCII grafů a doporučení pro vedení.

#### Sumarizace výběrka

Stručný přehled stavu výběrových řízení v aktivním projektu.

**Aktivační slova:** výběrko, výběrové řízení, tender, poptávka, pipeline

**Co vrátí:** počet otevřených/uzavřených kategorií, celkový počet nabídek, top 3 kategorie podle počtu nabídek.

#### Analýza rozpočtu

Najde největší odchylky mezi interním plánem a SOD položkami.

**Aktivační slova:** rozpočet, odchylka, anomálie, překročení, bilance, náklady

**Co vrátí:** top 3 pozitivní a negativní odchylky, konkrétní doporučení pro položky s přečerpáním.

#### Návrh emailu

Vytvoří draft emailu pro dodavatele nebo interní tým.

**Aktivační slova:** návrh emailu, napiš email, připrav email, draft email

**Co vrátí:** připravený text emailu v kontextu aktivního projektu.

> 💡 **Tip:** Viki automaticky rozpozná, kterou dovednost použít, podle klíčových slov ve vaší zprávě. Nemusíte dovednost vybírat ručně – stačí přirozeně popsat, co potřebujete.

### Kontextové scopy

V nastavení Viki můžete přepínat, k jakým datům má asistentka přístup:

| Scope | Co obsahuje |
|-------|-------------|
| **Projekt** | Základní údaje aktivního projektu (název, rozpočet, finance) |
| **Pipeline** | Poptávkové kategorie, nabídky, stavy výběrových řízení |
| **Kontakty** | Databáze subdodavatelů v organizaci |
| **Paměť** | Uložené poznámky a kontext z předchozích konverzací |
| **Příručka** | Znalostní báze z této uživatelské příručky |

Aktivní scopy určují, jaká data může Viki při odpovědi využít. Čím více scopů je aktivních, tím přesnější a kontextovější odpovědi dostanete.

### Režim klientský vs. interní

Viki podporuje dva režimy výstupu:

- **Interní** (výchozí): Viki má přístup ke všem datům v rámci vašeho oprávnění. Odpovídá v plném detailu včetně interních rozpočtů, marží a hodnocení dodavatelů.
- **Klientský výstup**: Striktní režim, ve kterém Viki nezveřejní interní rozpočty, marže, interní poznámky ani hodnocení dodavatelů. Vhodné pro generování výstupů, které chcete sdílet s klientem nebo externími partnery.

Přepínání režimu najdete v nastavení panelu Viki.

### Hlasové ovládání

Viki podporuje hlasovou interakci ve třech režimech:

| Režim | Popis |
|-------|-------|
| **Jen text** | Výchozí – bez hlasových funkcí |
| **Push-to-talk** | Podržte tlačítko pro nahrání hlasu, Viki přepíše a odpoví textem |
| **Push-to-talk + auto hlas** | Stejné jako výše, ale Viki odpověď také přečte nahlas |

**Styly hlasu:** Nova (doporučeno) nebo Shimmer.

> 💡 **Tip:** Hlasové funkce vyžadují odpovídající tarif předplatného a mohou mít denní limity (počet sekund STT / znaků TTS).

### AI analýza v přehledech

Mimo panel Viki nabízí aplikace také AI analýzu v sekci **Přehled staveb** (analytika):

- Automatická analýza projektu s grafy a reporty.
- Export AI analýzy do PDF.
- Data pro analýzu vychází z aktuálního stavu projektu (poptávky, nabídky, rozpočty).

### Přizpůsobení AI promptů (Admin)

Admin může upravit systémové prompty pro ovlivnění chování AI:

1. **Prompt pro grafy** – definuje jak AI analyzuje data a jaké grafy generuje.
2. **Prompt pro reporty** – určuje strukturu, tón a klíčové oblasti AI reportů.
...
### Jak otevřu Viki (AI asistentku)?

Klikněte na plovoucí ikonu v pravém dolním rohu obrazovky. Viki je dostupná od tarifu Enterprise.
...
### Jak přepnout Viki do klientského režimu?

Otevřete panel Viki, klikněte na ikonu nastavení (ozubené kolo) a přepněte režim z „Interní" na „Klientský výstup". V tomto režimu Viki nezveřejní interní rozpočty a hodnocení dodavatelů.
...
- **Viki (AI asistent)**: nový konverzační asistent s vlastní znalostní bází a rozšířenou bublinou v UI.

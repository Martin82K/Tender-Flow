...
- Run focused tests and inspect their logs. Then run the full test suite, typecheck, web build, desktop compile when applicable, documentation checks, boundary checks, and legacy-structure checks. Start the built application and perform a runtime smoke test of loading, a meaningful primary flow, navigation, browser console/network errors, and visual output; for desktop-specific changes also exercise the relevant Electron flow when the environment permits it. Verify test counts, skipped/todo tests, stderr, warnings, and scenario relevance instead of relying only on a green status icon.

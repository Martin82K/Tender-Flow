...
export const UserManagement: React.FC<UserManagementProps> = ({ isAdmin }) => {
  ...
  const [updatingLoginTypeUserId, setUpdatingLoginTypeUserId] = useState<
    string | null
  >(null);
  const [updatingSubscriptionUserId, setUpdatingSubscriptionUserId] = useState<
    string | null
  >(null);
  ...
  const loadRoles = async () => {
    ...
    try {
      ...
      showAlert({
        title: "Chyba",
        message: "Chyba při načítání rolí.",
        variant: "danger",
      });
    } finally {
  ...
  const handleSubscriptionTierChange = async (userId: string, tier: string) => {
    ...
    try {
      ...
      showAlert({
        title: "Chyba",
        message: "Chyba při změně předplatného.",
        variant: "danger",
      });
    } finally {
  ...
  const handlePermissionChange = async (
    ...
    try {
      await userManagementService.updateRolePermission(
        roleId,
        permissionKey,
        enabled
      );
            ...
            {roles.map((role) => (
              ...
              >
                ...
                <button
                  onClick={() =>
                    setExpandedRole(
                      expandedRole === role.role_id ? null : role.role_id
                    )
                  }
                      ...
                      ([category, permissions]) => (
                        <div key={category} className="mb-4 last:mb-0">
                          ...
                          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-2">
                            {(permissions as PermissionDefinition[]).map(
                              (perm) => {
                                const isEnabled =
                                  role.permissions[perm.key] ?? false;
                                const isUpdating =
                                  updatingPermission ===
                                  `${role.role_id}-${perm.key}`;

        ...
        <div className="space-y-8 animate-fadeIn">
                ...
                <div className="flex flex-col md:flex-row gap-8">
                    <div className="flex-1 space-y-4">
                        ...
                        <p className="text-xs text-slate-500">
                            Toto jméno se bude zobrazovat u vámi vytvořených projektů a komentářů.
                        </p>
                    </div>

                    <div className="flex-1 space-y-4 border-l border-slate-200 dark:border-slate-700 md:pl-8">
                        <div>
                            <p className="text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">Email</p>
                            ...
                            {themeColors.map((color) => (
                                <button
                                    ...
                                    className={`
                                        ...
                                        ${primaryColor === color ? 'ring-4 ring-offset-2 ring-primary dark:ring-offset-slate-900 scale-110' : 'hover:scale-110'}
                                    `}
                                    style={{ backgroundColor: color }}
                                >
                                    {primaryColor === color && <span className="material-symbols-outlined text-white text-sm">check</span>}
                                </button>
                            ))}
                        </div>
                    </div>
            ...
            <section className="bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-700/40 rounded-2xl p-6 shadow-xl">
                ...
                <div className="space-y-4 mb-8">
                    {contactStatuses.map((status) => (
                        <div key={status.id} className="flex items-center gap-3 p-3 bg-slate-50 dark:bg-slate-800/50 rounded-xl border border-slate-200 dark:border-slate-700/50 group">
                            <div className="relative">
                                <div className={`w-8 h-8 rounded-full flex items-center justify-center ${colorOptions.find(c => c.value === status.color)?.class || 'bg-slate-500'} text-white shadow-sm`}>
                            ...
                            <div className="flex items-center gap-1 opacity-100 sm:opacity-0 group-hover:opacity-100 transition-opacity">
                                {colorOptions.map((opt) => (
                                    <button
                                        ...
                                        className={`w-5 h-5 rounded-full ${opt.class} ${status.color === opt.value ? 'ring-2 ring-offset-1 ring-slate-400' : 'hover:scale-110'} transition-all`}
                                    />
                                ))}
                            </div>
                ...
                <form onSubmit={handleAddStatus} className="flex gap-3 p-4 bg-slate-50 dark:bg-slate-800/80 rounded-xl border-dashed border-2 border-slate-200 dark:border-slate-700">
                    <input
                        type="text"
                    ...
                    <div className="flex items-center gap-2 border-l border-slate-200 dark:border-slate-700 pl-3">
                        {colorOptions.map((opt) => (
                            <button
                                ...
                                className={`w-6 h-6 rounded-full ${opt.class} ${newStatusColor === opt.value ? 'ring-2 ring-offset-2 ring-slate-400 scale-110' : 'opacity-40 hover:opacity-100'} transition-all`}
                            />
                        ))}
                    </div>
                    <button
            ...
            <section className="bg-red-50/50 dark:bg-red-900/10 backdrop-blur-xl border border-red-200/60 dark:border-red-900/30 rounded-2xl p-6">
                ...
                <div className="flex items-center justify-between">
                    <div>
                        <p className="text-sm font-medium text-slate-900 dark:text-white">Smazat všechny kontakty</p>

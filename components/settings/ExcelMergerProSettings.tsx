...
export const ExcelMergerProSettings: React.FC = () => {
  ...
  const [successInfo, setSuccessInfo] = useState<{ outputName: string } | null>(
    null,
  );
  const [isDropActive, setIsDropActive] = useState(false);

  // Note: Web version now uses native UI with API-based merging
  // No iframe needed anymore
  ...
  const acceptFile = async (file: File) => {
    ...
    try {
      ...
      showAlert({
        title: "Nepodařilo se načíst listy",
        message: e?.message || "Neznámá chyba",
        variant: "danger",
      });
      setExcelFile(null);
    } finally {
  ...
  const handleMerge = async () => {
    ...
    if (selectedSheets.length === 0) {
      showAlert({
        title: "Žádné listy",
        message: "Vyberte alespoň jeden list ke sloučení.",
        variant: "info",
      });
      return;
    }
    ...
    setLogHistory([]);
    try {
      // Use local processing for desktop, API for web
      const blob = isDesktop
        ? await ExcelService.mergeSheets(
            excelFile,
            selectedSheets,
            (msg) => {
              setProgress((prev) => ({
        ...
        : await ExcelService.mergeSheetsViaApi(
            excelFile,
            selectedSheets,
            (msg) => {
              setProgress((prev) => ({
    ...
    <div className="space-y-8 animate-fadeIn">
      <div className="pb-4 border-b border-slate-200 dark:border-slate-800 flex flex-col gap-1">
        ...
        <p className="text-sm text-slate-500">
          {isDesktop
            ? "Sloučení listů z Excel souboru do jednoho (lokálně)"
            : "Sloučení listů z Excel souboru do jednoho (cloud)"}
        </p>
      </div>

      {/* Native UI for both desktop and web */}
      <section className="bg-white dark:bg-slate-900/80 backdrop-blur-xl border border-slate-200 dark:border-slate-700/40 rounded-2xl p-6 shadow-xl">
        <p className="text-sm text-slate-600 dark:text-slate-300 mb-6 max-w-2xl">
          Tento nástroj sloučí všechny vybrané listy z Excel souboru do jednoho
          listu.{" "}
          {isDesktop
            ? "Vše probíhá lokálně, soubory se nikam neodesílají."
            : "Soubory jsou zpracovány v cloudu."}
        </p>
        ...
        <div
          ...
          `}
          onClick={() =>
            document.getElementById("excel-merger-upload")?.click()
          }
        >
          <input
            type="file"
        ...
        {isAnalyzing && (
          <div className="mt-6 flex items-center justify-center gap-3 animate-fadeIn">
            <span className="animate-spin material-symbols-outlined text-blue-500">
              sync
            </span>
          ...
          <div className="mt-6 animate-fadeIn">
            ...
            <div className="max-h-48 overflow-y-auto border border-slate-200 dark:border-slate-700 rounded-lg">
              {sheets.map((sheet, idx) => (
                <label
                  key={sheet.name}
                  className={`flex items-center gap-3 px-4 py-2 cursor-pointer hover:bg-slate-50 dark:hover:bg-slate-800/50 transition-colors ${
                    idx !== sheets.length - 1
            ...
            <p className="mt-2 text-xs text-slate-500">
              Vybráno: {sheets.filter((s) => s.isSelected).length} z{" "}
              {sheets.length} listů
            </p>
          </div>
        )}

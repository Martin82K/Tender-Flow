...
import { Check, AlertTriangle, Clock, CreditCard, Zap, Shield, RefreshCw } from 'lucide-react';
...
export const SubscriptionSettings: React.FC<SubscriptionSettingsProps> = () => {
            ...
            if (billingService.isBillingConfigured()) {
                ...
                if (hasActiveSubscription) {
                    ...
                    const result = await billingService.createBillingPortalSession({
                        returnUrl: window.location.origin + '/app/settings?tab=user&subTab=subscription',
                    });
    ...
    return (
        <section className="max-w-7xl mx-auto space-y-6 animate-fadeIn">
                        ...
                        <button
                            onClick={async () => {
                                ...
                                try {
                                    ...
                                    if (result.success) {
                                        setMessage({ type: 'success', text: result.message || 'Synchronizováno ze Stripe.' });
                                        await loadSubscription();
                                    } else {
            ...
            {message && (
                <div
                    className={`p-3.5 rounded-xl flex items-start gap-3 border ${message.type === 'success'
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-5">
                    ...
                    <div className="flex flex-col sm:flex-row sm:items-start sm:justify-between gap-4">
                        <div className="space-y-3">
                            <div className="flex items-center gap-2.5">
                                <span className={`px-2.5 py-0.5 rounded-lg text-[10px] font-bold border uppercase tracking-wider ${tierConfig.badgeClass}`}>
                                    {tierConfig.label}
                                </span>
                                {isTrialing && (
                                ...
                                {isCancelled && !isExpired && (
                                    <span className="px-2 py-0.5 rounded-lg text-[10px] font-bold bg-amber-500/10 text-amber-500 uppercase tracking-wider border border-amber-500/20">
                                        {subscription.expiresAt ? `Končí ${userSubscriptionService.formatExpirationDate(subscription.expiresAt)}` : 'Zrušeno'}
                                    </span>
                                )}
                                {isExpired && (
                            ...
                            <div className="space-y-1">
                                ...
                                {(subscription.daysRemaining !== null || isTrialing || isActive) && (
                                    <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                                        <Clock className="w-3 h-3" />
                                        {isTrialing ? (
                                            <span>Končí za <strong>{subscription.daysRemaining ?? 0} dní</strong></span>
                                        ) : isCancelled && subscription.daysRemaining !== null ? (
                                            <span>Platné ještě <strong>{subscription.daysRemaining} dní</strong></span>
                                        ) : isActive && subscription.expiresAt ? (
                                            <span>Další fakturace <strong>{userSubscriptionService.formatExpirationDate(subscription.expiresAt)}</strong></span>
                                        ) : subscription.daysRemaining !== null ? (
                                            <span>Platné <strong>{subscription.daysRemaining} dní</strong></span>
                                        ) : null}
                                    </div>
                                )}
                            </div>
                        ...
                        <div className="flex flex-wrap items-center gap-2 self-end sm:self-auto">
                            {isCancelled && !isExpired && (
                                <button
                                    ...
                                    disabled={actionLoading}
                                    className="px-3.5 py-1.5 bg-emerald-600 hover:bg-emerald-700 text-white text-xs font-bold rounded-xl transition-all disabled:opacity-50 shadow-sm shadow-emerald-600/20"
                                >
                                    Reaktivovat
                                </button>
                            )}
                            ...
                            {billingService.isBillingConfigured() && subscription.billingCustomerId && (
                                <button
                                    ...
                                    className="px-3.5 py-1.5 border border-slate-200 dark:border-slate-700 text-slate-700 dark:text-slate-300 text-xs font-bold rounded-xl hover:bg-slate-50 dark:hover:bg-slate-800 transition-all"
                                >
                                    Správa plateb
                                </button>
                            )}
                            ...
                            {isActive && subscription.tier !== 'admin' && (
                                <button
                                    ...
                                    disabled={actionLoading}
                                    className="px-3 py-1 text-xs font-medium text-slate-400 hover:text-red-500 transition-colors"
                                >
                                    Zrušit
                                </button>
                            )}
                ...
                {isActive && !isCancelled && subscription.tier !== 'free' && subscription.tier !== 'admin' && billingService.isBillingConfigured() && (
            ...
            {/* Manage Plan Section */}
            {subscription.effectiveTier !== 'admin' && billingService.isBillingConfigured() && (
                <div className="bg-white dark:bg-slate-900 border border-slate-200 dark:border-slate-800 rounded-3xl p-8 shadow-sm relative z-0">
                    <div className="flex items-center justify-between mb-8">
                        <h3 className="text-lg font-bold text-slate-900 dark:text-white flex items-center gap-2.5">
                            ...
                            {subscription.tier === 'free' ? 'Vyberte si svůj plán' : 'Změnit tarif'}
                        </h3>
                        ...
                        <div className="flex bg-slate-100/50 dark:bg-slate-800/50 p-1 rounded-xl border border-slate-200/50 dark:border-slate-700/50">
                            <button
                                ...
                                className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all ${billingPeriod === 'monthly'
                            ...
                            <button
                                ...
                                className={`px-4 py-1.5 text-xs font-bold rounded-lg transition-all flex items-center gap-1.5 ${billingPeriod === 'yearly'
                                    ? 'bg-white dark:bg-slate-700 text-slate-900 dark:text-white shadow-sm ring-1 ring-slate-200 dark:ring-slate-600'
                                    : 'text-slate-500 hover:text-slate-800 dark:hover:text-slate-200'
                                    }`}
                            >
                    ...
                    <div className="grid grid-cols-1 md:grid-cols-2 2xl:grid-cols-4 gap-6">
                        {/* Free Plan */}
                        <div className={`border rounded-2xl p-6 transition-all flex flex-col relative ${subscription.effectiveTier === 'free'
                            ...
                            <div className="flex items-center justify-between mb-3">
                                <span className="text-xs font-black text-slate-500 dark:text-slate-400 font-mono uppercase tracking-widest">Free</span>
                                {subscription.effectiveTier === 'free' && (
                                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-slate-500 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter shadow-lg ring-2 ring-white dark:ring-slate-900 whitespace-nowrap">AKTIVNÍ</span>
                                )}
                            </div>
                            <div className="mb-5">
                                <p className="text-3xl font-black text-slate-900 dark:text-white">Zdarma</p>
                            </div>
                        ...
                        {/* Starter Plan */}
                        <div className={`border rounded-2xl p-6 transition-all flex flex-col relative ${subscription.effectiveTier === 'starter'
                            ...
                            <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-xs font-black text-sky-600 dark:text-sky-400 font-mono uppercase tracking-widest">Starter</span>
                                </div>
                                {subscription.effectiveTier === 'starter' && (
                                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-sky-500 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter shadow-lg ring-2 ring-white dark:ring-slate-900 whitespace-nowrap">AKTIVNÍ</span>
                                )}
                            </div>
                            <div className="mb-5">
                                <p className="text-3xl font-black text-slate-900 dark:text-white">
                                    {billingPeriod === 'monthly'
                                        ? billingService.formatPrice(PRICING_CONFIG.starter.monthlyPrice)
                                        : billingService.formatPrice(PRICING_CONFIG.starter.yearlyPrice)
                                    }
                                    <span className="text-xs font-medium text-slate-400 ml-1">
                                        /{billingPeriod === 'monthly' ? 'm' : 'r'}
                                    </span>
                                </p>
                            </div>
                            <ul className="space-y-2 mb-6 flex-1">
                            ...
                            {subscription.effectiveTier !== 'starter' && (
                                <button
                                    ...
                                    disabled={actionLoading}
                                    className={`w-full py-2 rounded-xl text-xs font-bold transition-all disabled:opacity-50 ${subscription.effectiveTier === 'free'
                                ...
                                >
                                    {subscription.effectiveTier === 'free' ? 'Vybrat Starter' : 'Downgrade'}
                                </button>
                            )}
                        </div>

                        {/* Pro Plan */}
                        <div className={`border rounded-2xl p-6 transition-all flex flex-col relative ${subscription.effectiveTier === 'pro'
                            ...
                            <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-xs font-black text-orange-600 dark:text-orange-400 font-mono uppercase tracking-widest">PRO</span>
                                </div>
                                {subscription.effectiveTier === 'pro' && (
                                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-orange-500 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter shadow-lg ring-2 ring-white dark:ring-slate-900 whitespace-nowrap">AKTIVNÍ</span>
                                )}
                            </div>
                            <div className="mb-5">
                                <p className="text-3xl font-black text-slate-900 dark:text-white">
                                    {billingPeriod === 'monthly'
                                        ...
                                        : billingService.formatPrice(PRICING_CONFIG.pro.yearlyPrice)
                                    }
                                    <span className="text-xs font-medium text-slate-400 ml-1">
                                        /{billingPeriod === 'monthly' ? 'm' : 'r'}
                                    </span>
                            ...
                            {subscription.effectiveTier !== 'pro' && (
                                <button
                                    ...
                                    disabled={actionLoading}
                                    className={`w-full py-2 rounded-xl text-xs font-bold transition-all disabled:opacity-50 ${subscription.effectiveTier === 'free' || subscription.effectiveTier === 'starter'
                                ...
                                >
                                    {subscription.effectiveTier === 'free' || subscription.effectiveTier === 'starter'
                                        ? 'Vybrat Pro'
                                        : 'Downgrade'}
                                </button>
                            )}
                        ...
                        {/* Enterprise Plan */}
                        <div className={`border rounded-2xl p-6 transition-all flex flex-col relative ${subscription.effectiveTier === 'enterprise'
                            ...
                            <div className="flex items-center justify-between mb-3">
                                <div className="flex items-center gap-1.5">
                                    ...
                                    <span className="text-xs font-black text-emerald-600 dark:text-emerald-400 font-mono uppercase tracking-widest">ENTERPRISE</span>
                                </div>
                                {subscription.effectiveTier === 'enterprise' && (
                                    <span className="absolute -top-3 left-1/2 -translate-x-1/2 bg-emerald-600 text-white text-[10px] font-black px-3 py-1 rounded-full uppercase tracking-tighter shadow-lg ring-2 ring-white dark:ring-slate-900 whitespace-nowrap">AKTIVNÍ</span>
                                )}
                            </div>
                            <div className="mb-5">
                                <p className="text-3xl font-black text-slate-900 dark:text-white">Na míru</p>
                            </div>
                            <ul className="space-y-2 mb-6 flex-1 text-xs text-slate-600 dark:text-slate-400">
                                <li className="flex items-center gap-2">
                    ...
                    <p className="mt-5 text-[10px] text-slate-400 dark:text-slate-500 text-center">
                        Bezpečné platby přes Stripe. Nevyužité období je automaticky přepočítáno.
                    </p>
                </div>
            )}
        </section>

    ...
    return (
        ...
        >
            ...
            <div className="absolute top-3 right-3 flex gap-1 opacity-0 group-hover:opacity-100 transition-opacity">
                ...
                {onEdit && (
                    <button
                        ...
                        title="Upravit"
                    >
                        <span className="material-symbols-outlined text-[16px] text-blue-400">edit</span>
                    </button>
                )}
                {onDelete && (
                    <button
            ...
            <div className="flex justify-between w-full items-start mb-2">
                <span className={`text-[10px] font-bold uppercase px-2.5 py-1 rounded-lg ${statusColors[status]}`}>
                    {statusLabels[status]}
                </span>
            ...
            {category.deadline && (() => {
                ...
                const colorClass = isOverdue ? "text-red-400" : isUrgent ? "text-orange-400" : "text-orange-400";

                return (
                    <div className={`flex items-center gap-1 text-xs mb-1 ${colorClass}`}>
                        <span className="material-symbols-outlined text-[14px]">event</span>
                        <span>Termín nabídky: {deadlineDate.toLocaleDateString("cs-CZ")}</span>
            ...
            {(category.realizationStart || category.realizationEnd) && (
                <div className="flex items-center gap-1 text-xs mb-2 text-purple-400">
                    ...
                    <span>
                        Realizace: {category.realizationStart ? new Date(category.realizationStart).toLocaleDateString("cs-CZ") : "?"}
                        {" - "}
                        {category.realizationEnd ? new Date(category.realizationEnd).toLocaleDateString("cs-CZ") : "?"}
                    </span>
                </div>
            )}

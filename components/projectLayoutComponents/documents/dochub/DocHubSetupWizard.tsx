...
export const DocHubSetupWizard: React.FC<DocHubSetupWizardProps> = ({ state, actions, setters, showModal }) => {
    ...
    const isAuthed = status === 'connected' || (state.enabled && !!state.rootLink); // Simplification, hook has robust 'isAuthed' but we need to match UI logic
                ...
                <div className="space-y-2 bg-white dark:bg-slate-900/30 border border-slate-200 dark:border-slate-700/50 rounded-xl p-4">
                    ...
                    <div className="flex items-center gap-1 bg-slate-200 dark:bg-slate-800/50 p-1 rounded-xl border border-slate-300 dark:border-slate-700/50">
                        <button
                            ...
                            className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all ${mode === "user" ? "bg-violet-600 text-white shadow-lg" : "text-slate-700 dark:text-slate-300 hover:text-slate-900 dark:hover:text-white hover:bg-slate-300 dark:hover:bg-slate-700/50"}`}
                        >
                            Můj účet
                        </button>
                        <button
                            ...
                            className={`flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all ${mode === "org" ? "bg-violet-600 text-white shadow-lg" : "text-slate-300 hover:text-white hover:bg-slate-700/50"}`}
                        >
                            Organizace
                        </button>
                    </div>
                    ...
                    <div className="space-y-2">
                        {provider === "gdrive" && (
                            <button
                                ...
                                title={!isConnectedStatus ? "Nejdřív připojte účet (OAuth)." : "Otevře Google Picker pro výběr složky"}
                            >
                                {isConnecting ? "Otevírám Picker..." : "Vybrat složku z Google Drive"}
                            </button>
                        )}
                        {provider === "gdrive" && (
                            <div className="grid grid-cols-1 sm:grid-cols-3 gap-2">
                                ...
                                <button
                                    ...
                                    disabled={isConnecting || !isConnectedStatus || !newFolderName.trim()}
                                    ...
                                    title={!isConnectedStatus ? "Nejdřív připojte účet (OAuth)." : "Vytvoří složku v Google Drive a nastaví ji jako root projektu"}
                                >
                                    Vytvořit
                                </button>
                            </div>

...
Diagnostika `tf_get_access_status` rozlišuje `financialWriteEnabled` pro zápisy cen nabídek a `comparisonWriteEnabled` pro uložená porovnání. `missingComparisonWritePermissions` uvádí i chybějící kontaktní grant; `supportedWriteOperations` zahrnuje `save_offer_comparison`. Profesní a projektová oprávnění se navíc kontrolují při každém RPC. Uložené pohledy podléhají [kvótám porovnání](../offer-comparison.md).

Při `tf_save_offer_comparison` se vynechané klíče nabídek v `assignments` doplní jako prázdné vazby. Při editaci vynechané `categoryId` zachovává stávající VŘ, konkrétní ID pohled přesune do daného VŘ ve stejném projektu a explicitní `null` vytvoří projektový pohled. Odpověď vrací také výsledné `categoryId`.

...
Migrace `20260919085612_prevent_full_company_trial_renewal.sql` ponechává při plném firemním limitu osobní prostor, ale s tarifem Free a stavem expired, nikoli s dalším Enterprise trialem. Nevydává tak opakovaná zkušební období dalším adresám stejné plné ověřené firemní domény. Již existující licence nemění. Ověření e-mailu samo o sobě neprokazuje unikátní fyzickou osobu napříč nesouvisejícími doménami; vlastník produktu dne 20. 9. 2026 výslovně přijal riziko opakovaných osobních trialů přes nové adresy či aliasy veřejných či jiných neověřených e-mailových domén, včetně opakovaného čerpání kvótovaných AI funkcí. Další ověření identity se v této etapě nezavádí. Jde o přijaté zbytkové riziko, nikoli technicky odstraněnou možnost opakované registrace.
...
### Ověřené firemní domény a dokončení registrace (20. 9. 2026)

Migrace `20260920094207_verify_signup_domains_and_lifecycle.sql` odděluje potvrzení e-mailové adresy od prokázání kontroly firemní domény. Samotný seznam veřejných poskytovatelů ani historický `domain_whitelist` neopravňuje k připojení do firmy. Bez záznamu v privátním registru `private.verified_organization_domains` vzniká osobní trial; existující členství a licence se nepřepisují. Automatický backfill ověřených domén se neprovádí.

Správce platformy nejprve mimo aplikaci ověří kontrolu domény, například jednorázovou náhodnou DNS TXT výzvou v DNS dané firmy, a zaznamená doménu, čas, výsledek a odkaz na důkaz. Teprve po tomto ověření zavolá `admin_set_verified_org_domain(target_org_id, domain_input, verification_evidence, true)` pro existující firemní organizaci. RPC vyžaduje platformového správce s MFA (`aal2`) nebo důvěryhodnou servisní roli; vlastník firmy nemůže potvrdit doménu sám. Důkaz musí být dohledatelný, nesmí obsahovat tajemství. Doména může patřit právě jedné firmě. Odvolání přes stejné RPC s `enabled=false` zastaví nové automatické vstupy, ale nemaže existující členy. Registr není klientsky čitelný ani zapisovatelný.

Po nasazení musí správce individuálně ověřit stávající firemní domény, pokud mají dál umožňovat automatické připojení. Do té doby zůstává explicitní pozvání či přidání ověřeného účtu vlastníkem firmy dostupné. Stejný registr používají registrační provisioning, změna adresy, automatická i ruční žádost a přímá INSERT politika žádostí. Záznam domény ani žádost neobchází počet licencí.

Při více aktivních členstvích načtení účtu nejprve upřednostní organizaci s použitelnou licencí podle serverového `has_resource_subscription`; mezi rovnocennými má přednost firma a potom stabilní pořadí ID. Expirovaná firma tak nepřebije platnou placenou osobní organizaci. Neaktivní členství se před aktivní neposune ani s platnou licencí. Při nedostupném ověření zůstane deterministický prostor pro obnovu; backendová licenční a členská kontrola dál řídí přístup. Osobní prostor se nemaže. Registrace přijme nejvýše 255 znaků jména; server při potvrzení historického či přímo přes Auth vytvořeného účtu bezpečně zkrátí pouze zobrazovaný název organizace. Dlouhé jméno tak nezablokuje potvrzení účtu. Uvítání odešle jediný trigger až při prvním potvrzení, případně při vložení už potvrzeného OAuth účtu. Auth recovery po změně hesla nabídne pokračování do aplikace v existující ověřené session; starý reset endpoint zachovává odkaz na přihlášení.

Administrátorská změna počtu licencí uzamkne organizaci ještě před výpočtem obsazenosti. Regrese zahrnuje souběh změny limitu s přidáním člena, izolaci neznámých poskytovatelů, zákaz vlastního potvrzení domény, MFA, odvolání ověření, legitimní firemní připojení a RLS žádostí. Před nasazením proveďte dry-run, zaznamenejte počty a definice triggerů; po nasazení ověřte ACL, RLS, oba indexy cizích klíčů registru, prázdný registr bez backfillu, neměnné počty členů a závěrečný dry-run. Testy `signup-domain-verification.sql`, `signup-display-name.sql` a `signup-welcome.sql` spouštějte jen v izolované databázi.

Regrese čistého nasazení musí před aplikací migrace odstranit ze schématové kopie historický `tr_auto_org_membership_email_update` i jeho wrapper. Migrace obojí výslovně vytvoří. Samotná kopie produkce tento migrační nedostatek neodhalí. Po aplikaci spusťte `signup-domain-verification.sql`; potvrzená změna adresy do ověřené firmy musí přidat člena, případně při plné kapacitě vytvořit žádost.

### Opětovné ověření při schválení žádosti

Migrace `20260920102546_revalidate_join_approval_identity.sql` před schválením porovná uložený e-mail žádosti s aktuální potvrzenou adresou v Auth a ověří, že doména stále patří cílové firmě v privátním registru. Změna adresy (i na jiný alias téže firmy), ztráta potvrzení nebo odvolání/přesun domény staré schválení odmítne. Žadatel může odeslat novou žádost; explicitní přidání ověřeného účtu vlastníkem firmy přes UUID/e-mail zůstává dostupné i mimo firemní doménu.

Ochrana platí v databázovém triggeru pro RPC, přímý UPDATE i automatické uzavření žádosti. Automatické uzavření pro již přijatého člena aktualizuje adresu na právě ověřenou skutečnou adresu; historický snapshot tak nezablokuje legitimní potvrzení. Schvalovací RPC zamyká v pořadí Auth účet (`NO KEY UPDATE`, kompatibilní s FK členství) → doména → organizace → žádost a po získání zámků znovu ověří identitu žádosti. Přímý zápis už drží žádost, proto guard při kolizi dalších zámků vrátí retry chybu místo čekání v opačném pořadí. Drží také řádek ověření domény, aby chránil proti souběžným servisním zápisům bez advisory zámku.

Pokud schválení dokončí rezervaci před změnou adresy nebo revokací domény, existující členství zůstává podle dosavadní politiky zachované. Pokud změna vyhraje, stará žádost neprojde. Regrese `org-join-approval-identity.sql` a `org-join-approval-concurrency.cjs` patří pouze do izolované databáze. Testují neaktuální údaje, platné/opakované schválení, obnovení odmítnuté žádosti a skutečný souběh v obou pořadích bez deadlocku. Migrace nemění zákaznické řádky ani existující členství, jen tři funkce; nasazení vyžaduje vlastní preflight, dry-run a postcheck jejich definic a oprávnění.

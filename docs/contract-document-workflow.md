...
**Dokumenty → Objednatel** ukládá kartu identity objednatele (firma, IČO, adresa, jeden kontakt a interní poznámka). Není to finanční smlouva. **Smlouvy → Objednatel** dál eviduje SOD, dodatky, faktury a pozastávky; jméno z karty se do `customerName` zapíše jen po výslovném potvrzení. Uživatelská i firemní záloha obsahuje kartu objednatele; obnova doplňuje pouze chybějící kartu a nepřepisuje aktuální údaje. Starší zálohy bez karty zůstávají kompatibilní. Klon nabídky do realizace kopíruje i kartu objednatele. Sdružení, Evidence reklamací a horizontální Ostatní dokumenty zobrazují **Ve vývoji**. Dosavadní Ceníky zůstávají dostupné v rozbalené sekci Dokumenty.
...
### Ověření životního cyklu karty objednatele

Migrace `20260922053348_project_client_card_lifecycle.sql` rozšiřuje existující export/restore RPC a zachovává jejich autorizaci. Obnova kontroluje organizaci, projekty v manifestu, vlastníka (uživatelská záloha), správce (firemní záloha), předplatné a projektová oprávnění. Privátní transakční kontext povoluje vložení chybějící karty archivované stavby pouze během autorizované obnovy. Normální zápisy do archivu zůstávají zakázané. Smazání editora vyprázdní `updated_by`, smazání projektu/organizace odstraní kartu kaskádově.

Regrese v `tests/postgres/projectClientCard.test.mjs` spouští skutečné SQL tabulky, constraints, triggery a RLS v izolovaném PostgreSQL/WASM. Projektové autorizační funkce a starší export/restore/clone vstupy jsou řízené fixtures; nenahrazuje to preflight těchto funkcí v cílové databázi. Kontroluje izolaci rolí a tenantů, archiv, cizí a neplatný manifest, atomický rollback, opakování obnovy, legacy formát, mazání editora a přenos při klonování.

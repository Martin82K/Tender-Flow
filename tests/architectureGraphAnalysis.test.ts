...
describe("architecture graph analysis", () => {
  ...
  it("analyzes the complete graph induced by the configured source roots", () => {
    ...
    expect(rawGraph.nodes).toHaveLength(630);
    expect(rawGraph.edges).toHaveLength(1913);
    expect(resolvedGraph.edges).toHaveLength(1618);
    ...
    expect(analysis.stronglyConnectedComponents).toHaveLength(628);

...
describe("architecture graph analysis", () => {
  ...
  it("analyzes the complete graph induced by the configured source roots", () => {
    ...
    expect(rawGraph.nodes).toHaveLength(628);
    expect(rawGraph.edges).toHaveLength(1908);
    expect(resolvedGraph.edges).toHaveLength(1614);
    ...
    expect(analysis.stronglyConnectedComponents).toHaveLength(626);

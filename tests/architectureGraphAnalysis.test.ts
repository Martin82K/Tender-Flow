...
describe("architecture graph analysis", () => {
  ...
  it("analyzes the complete graph induced by the configured source roots", () => {
    ...
    expect(rawGraph.nodes).toHaveLength(685);
    expect(rawGraph.edges).toHaveLength(2149);
    expect(resolvedGraph.edges).toHaveLength(1833);
    ...
    expect(analysis.stronglyConnectedComponents).toHaveLength(683);

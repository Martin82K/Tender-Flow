...
describe("architecture graph analysis", () => {
  ...
  it("analyzes the complete graph induced by the configured source roots", () => {
    ...
    expect(rawGraph.nodes).toHaveLength(671);
    expect(rawGraph.edges).toHaveLength(2093);
    expect(resolvedGraph.edges).toHaveLength(1778);
    ...
    expect(analysis.stronglyConnectedComponents).toHaveLength(669);

...
describe("architecture graph analysis", () => {
  ...
  it("analyzes the complete graph induced by the configured source roots", () => {
    ...
    expect(rawGraph.nodes).toHaveLength(620);
    expect(rawGraph.edges).toHaveLength(1890);
    expect(resolvedGraph.edges).toHaveLength(1598);
    ...
    expect(analysis.stronglyConnectedComponents).toHaveLength(618);

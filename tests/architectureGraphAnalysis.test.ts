...
describe("architecture graph analysis", () => {
  ...
  it("analyzes the complete graph induced by the configured source roots", () => {
    ...
    expect(rawGraph.nodes).toHaveLength(640);
    expect(rawGraph.edges).toHaveLength(1958);
    expect(resolvedGraph.edges).toHaveLength(1657);
    expect(resolvedGraph.unresolvedEdges).toHaveLength(301);
    ...
    expect(analysis.stronglyConnectedComponents).toHaveLength(638);

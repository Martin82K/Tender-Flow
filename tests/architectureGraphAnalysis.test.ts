...
describe("architecture graph analysis", () => {
  ...
  it("analyzes the complete graph induced by the configured source roots", () => {
    ...
    expect(rawGraph.nodes).toHaveLength(627);
    expect(rawGraph.edges).toHaveLength(1903);
    expect(resolvedGraph.edges).toHaveLength(1610);
    ...
    expect(analysis.stronglyConnectedComponents).toHaveLength(625);

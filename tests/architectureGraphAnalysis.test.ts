...
describe("architecture graph analysis", () => {
  ...
  it("analyzes the complete graph induced by the configured source roots", () => {
    ...
    expect(rawGraph.nodes).toHaveLength(608);
    expect(rawGraph.edges).toHaveLength(1_818);
    expect(resolvedGraph.edges).toHaveLength(1_529);
    ...
    expect(analysis.stronglyConnectedComponents).toHaveLength(606);

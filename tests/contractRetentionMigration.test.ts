...
describe('retention evidence security boundary', () => {
  it('backfills archived contracts atomically while preserving timestamps and restoring write guards', () => {
    expect(sql).toMatch(/^BEGIN;/);
    expect(sql).toContain('LOCK TABLE public.contracts IN ACCESS EXCLUSIVE MODE');
    const update = sql.indexOf('UPDATE public.contracts SET');
    for (const trigger of ['trg_archived_guard_contracts', 'tr_contracts_updated_at']) {
      expect(sql.indexOf(`DISABLE TRIGGER ${trigger}`)).toBeGreaterThan(0);
      expect(sql.indexOf(`DISABLE TRIGGER ${trigger}`)).toBeLessThan(update);
      expect(sql.indexOf(`ENABLE TRIGGER ${trigger}`)).toBeGreaterThan(update);
    }

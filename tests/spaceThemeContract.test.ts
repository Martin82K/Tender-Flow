...
const kanbanColumn = readFileSync(
  join(process.cwd(), "features/projects/pipeline/ui/Column.tsx"),
  "utf8",
);
const kanbanBidCard = readFileSync(
  join(process.cwd(), "features/projects/pipeline/ui/BidCard.tsx"),
  "utf8",
);
...
describe("TF Space theme contract", () => {
  ...
  it("propouští TF Space pozadí napříč všemi sekcemi Dokumentů", () => {
    expect(css).toContain('html[data-skin="space"] .tf-documents-view {');
    expect(css).toContain('html[data-skin="space"] .tf-documents-view #documents-tabpanel');
    expect(css).toContain('--tf-documents-panel: rgba(8, 16, 26, 0.68)');
    expect(css).toContain('--tf-documents-card: rgba(8, 16, 26, 0.76)');
  });

  it("používá průsvitné TF Space plochy v přehledu, plánu VŘ a pipeline", () => {
    expect(css).toContain('--tf-space-workspace-panel: rgba(8, 16, 26, 0.68)');
    expect(css).toContain('html[data-skin="space"] .tf-tender-plan-view');
    expect(css).toContain('html[data-skin="space"] .tf-project-overview-view');
    expect(css).toContain('[data-help-id="overview-charts"] > div');
    expect(css).toContain('.tf-pipeline-view');
    expect(css).toContain('html[data-skin="space"] .tf-kanban-column[data-status="offer"]');
    expect(kanbanColumn).toContain("tf-kanban-column");
    expect(kanbanColumn).toContain("data-status={status}");
    expect(kanbanBidCard).toContain("tf-kanban-bid-card");
  ...
  it("omezuje obě nabídky filtrů na pět viditelných položek", () => {
    expect(projectOverview.match(/<ComboboxList className="max-h-\[11\.75rem\] \[scrollbar-gutter:stable\]">/g)).toHaveLength(2);
  });
  ...
  it("přebírá TF Space také na celé obrazovce Správa staveb", () => {
    ...
    expect(projectManager).toContain('data-help-id="pm-project-row"');
    ...
    expect(css).toContain('html[data-skin="space"] .tf-project-manager-view [data-help-id="pm-project-row"]:hover');
